...
This Hands on Exercise will focus on how to plot functional and truthful choropleth maps by using an R package call **tmap** package
...
The content of `mpsz` can be examine by using the code chunk below.
...
The below code chunk import *respopagsex2011to2020.csv* file into RStudio and save the file into an R dataframe called *popagsex*.
...
Before perform the georelational join, one extra step is required to convert the values in PA and SZ fields to uppercase. This is because the values of PA and SZ fields are made up of upper- and lowercase. On the other, hand the SUBZONE_N and PLN_AREA_N are in uppercase.
...
tm_shape(mpsz_pop2020)+
  ...
  tm_borders(alpha = 0.5) +
  tm_compass(type="8star", size = 2) +
  tm_scale_bar() +  # No width argument
...
# Check for any NA values across the entire dataset
colSums(is.na(mpsz_pop2020))
```

```{r}
mpsz_pop2020_clean <- mpsz_pop2020

tm_shape(mpsz_pop2020_clean) +
  tm_fill("DEPENDENCY",
          style = "quantile",
          palette = "Blues",
          thres.poly = 0) + 
  tm_facets(by="REGION_N", 
            free.coords=TRUE) +  # Removed drop.shapes parameter
