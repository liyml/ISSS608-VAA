...
While traditional methods like histograms, probability density curve (pdfs), boxplots, notch plots, and violin plots have long been staples for visualizing distributions in statistics, modern data visualization offers exciting new alternatives. In this chapter, we'll explore two contemporary approaches that have gained popularity: ridgeline plots and raincloud plots. We'll demonstrate how to create these innovative visualizations using ggplot2 and its extended packages, building upon the foundational techniques covered in chapter 1.
...
We will start with plot a Half-Eye graph by using [`stat_halfeye()`](https://mjskay.github.io/ggdist/reference/stat_halfeye.html) of **ggdist** package.
...
We will then add the second geometry layer using [`geom_boxplot()`](https://r4va.netlify.app/chap09) of ggplot2. This produces a narrow boxplot. We reduce the width and adjust the opacity.
...
The [`coord_flip()`](https://ggplot2.tidyverse.org/reference/coord_flip.html) of ggplot2 package will be used to flip the raincloud chart horizontally to give it the raincloud appearance. At the same time, `theme_economist()` of ggthemes package is used to give the raincloud chart a professional publishing standard look.
